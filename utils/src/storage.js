...
/**
 * Reads and writes JSON files on the user's local disk. Every method is
 * static, so the class is used directly (`Storage.readJSON(...)`) without
 * ever needing to instantiate it.
 */
class Storage {
  /**
   ...
   */
  static async readJSON(filePath, fallback) {
    try {
  ...
  /**
   * Writes an object to disk as pretty-printed JSON, atomically: the data
   * is written to a temp file in the same directory, then renamed into
